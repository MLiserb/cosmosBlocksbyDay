...
function createChart(data, useLogScale = false) {
...
function setDefaultDates() {
  ...
  // Set start date to August 1, 2022
...
async function loadAndDisplayData() {
  ...
  try {
    ...
    if (chartData && chartData.length > 0) {
      createChart(chartData, useLogScale);
    } else {
...
function setupEventListeners() {
  ...
  logScaleCheckbox.addEventListener('change', () => {
    if (chartData) {
      createChart(chartData, logScaleCheckbox.checked);
    }
  });
}
